...
SUBSETS = [
    ...
    '___earth_science',
    'driving_rule',
    'basic_traditional_medicine',
    'clinical_traditional_medicine',
...
def parse_args():
    parser = argparse.ArgumentParser(description='Run B-REASO')
    parser.add_argument(
        '--backend',
    ...
    parser.add_argument(
        ...
        type=str,
        default='ALL',
        help="The subsets of B-REASO (splited by comma). Default is 'ALL'.",
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--timeout', type=float, default=15.0, help='Timeout for API based backend.'
    )
    parser.add_argument(
        '--max_retries', type=int, default=75, help='Max retries for API based backend.'
    )
...
def format_problem(
    ...
    topic_line: str = 'নিম্নলিখিত বহুনির্বাচনী প্রশ্নটি বাংলাদেশের একটি পরীক্ষা থেকে নেওয়া হয়েছে, এবং উত্তরটি বিকল্পগুলির মধ্যে একটি।',
    ...
    if args.subsets == 'ALL':
        ...
        for subset in subsets:
            if subset not in SUBSETS:
                raise ValueError(f'{subset} is not an available subset of B-REASO.')
    ...
    for subset_name in subsets:
        ...
        if args.backend == 'openai' or args.backend == 'custom_api':
            ...
            outputs = model.generate(
                test_data, prefill='\nসঠিক উত্তর:(', apply_chat_template=args.apply_chat_template
            )
