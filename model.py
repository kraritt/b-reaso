...
class HFLM_vLLM(LM):
    def __init__(
        ...
        self.sampling_params = SamplingParams(
            temperature=self.temperature, max_tokens=self.max_tokens, stop=['প্রশ্ন: ']
        )

    def generate(self, dataset, prefill='সঠিক উত্তর: (', apply_chat_template=True):
...
class Anthropic_LM(LM):
    def __init__(self, model_name, max_tokens, temperature, api_key, timeout=15.0, max_retries=75):
        super().__init__(max_tokens, temperature)
